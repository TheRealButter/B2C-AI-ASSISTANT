...
const Navigation: React.FC<NavigationProps> = ({ currentView, onNavigate }) => {
  const navItems = [
    ...
    { id: 'PLACEMENT', label: 'MISSION AI', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
    { id: 'BUDGET', label: 'BUDGET', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1' },
  ];

  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-[100]">
      <nav className="glass-nav rounded-[28px] px-8 py-4.5 flex justify-between items-center shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)] border border-white/5">
        {navItems.map((item) => (
          <button 
            key={item.id}
